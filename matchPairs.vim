...
fu! SearchForMatchingWord(pairsIndex, isBackwardSearch)
    let starttag = g:pairsToMatch[a:pairsIndex][0]
    let midtag = ''
    let endtag = g:pairsToMatch[a:pairsIndex][1]
    let flags = 'nW'
...
fu! HighlightPairs(line, column, wordIndex, pairsIndex, isBackward)
	...
	let word = g:pairsToMatch[a:pairsIndex][a:isBackward]
    ...
    exe '2match MatchParen ^' . subStrL . '\%' . line('.') . 'l\%' . col('.') . 'c' . subStrR . '\c\|' .
    	\ '\%' . a:line . 'l\%' . a:column . 'c'. g:pairsToMatch[a:pairsIndex][a:isBackward?0:1] . '\c^'
